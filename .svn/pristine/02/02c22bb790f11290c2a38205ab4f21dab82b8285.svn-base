...
//		.logout(request, response, null);
	...
	public ModelAndView patientRegistration(Model model,HttpSession session) {
	...
	public ModelAndView claimInitiation(Model model,HttpSession session) {
		String token = (String) session.getAttribute("loginToken");
		if(token!=null) {	
			model.addAttribute("role",(Integer) session.getAttribute("role"));
	...
	public ModelAndView claimInitiationProcess(Model model,HttpSession session) {
		String token = (String) session.getAttribute("loginToken");
		if(token!=null) {	
			model.addAttribute("role",(Integer) session.getAttribute("role"));
	...
	public @ResponseBody List<LoginUserHospitalResponse> getCurrentHosp(Model model,HttpSession session,@RequestParam String hid,HttpServletRequest request) {
		...
		try {
			...
			response= userAuthenticationService.getCurrentHosp(token,(String)session.getAttribute("loginId"),(String)session.getAttribute("loginUserstateCode"));
			logger.info("getCurrentHosp Response"+response);
			if(!ObjectUtils.isEmpty(response)) {	
				JSONObject userJson= new JSONObject(response);
				if(userJson.getString("status")!=null 
						&& userJson.getString("status").equalsIgnoreCase("SUCCESS")
